...
For SQL views specifically, RESTier auto-maps EF Core `[Keyless]` / `HasNoKey()` / `ToView()` types to
read-only OData function imports — no custom model code required. See [Keyless Views](./keyless-views).
EF6 users still need to define a primary key on the view's CLR type or omit the view from their service.
