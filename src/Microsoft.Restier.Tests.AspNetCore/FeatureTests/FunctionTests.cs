...
namespace Microsoft.Restier.Tests.AspNetCore.FeatureTests
{
    ...
    public class FunctionTests : RestierTestBase
    {
        ...
        {
            var books = new List<Book> { new Book { Id = Guid.NewGuid(), Title = "War and Peace" }, new Book { Id = Guid.NewGuid(), Title = "Of Mice and Men" } }.AsQueryable();
            var response = await RestierTestHelpers.ExecuteTestRequest<LibraryApi, LibraryContext>(HttpMethod.Post, resource: "/Books/DiscontinueBooks()", payload: new { books });
            var content = await TestContext.LogAndReturnMessageContentAsync(response);
            ...
            results.Should().NotBeNull();
            results.Items.Should().NotBeNullOrEmpty();
            results.Items.Count.Should().BeOneOf(2);
